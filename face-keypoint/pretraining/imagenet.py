...
parser.add_argument('--size', type=int, default=(3, 32, 64, 128, 256), nargs='*',
...
def main():
    ...
    class Capsule(nn.Module):
        ...
        def __init__(self):
            super().__init__()
            nb_of_classes = 1000 #33  # 970 (vid) or 35 (vid obj) or 33 (imgs)
            self.inner_model = Model()
            self.classifier = nn.Linear(512, 1000)
        def forward(self, x):
            for e in self.inner_model.encoder:
                x = e(x)
